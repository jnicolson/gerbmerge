...
Config = {
    ...
    'bottommargin': 0,
    # List of X,Y co-ordinates at which to draw fiducials
    'fiducialpoints': None,
...
def parseToolList(fname):
    ...
    for line in fid.xreadlines():
        ...
        if tool in TL:
            raise RuntimeError(
                "Tool '%s' defined more than once in tool list file '%s'" % (tool, fname))
    ...
    if CP.has_section('Options'):
        for opt in CP.options('Options'):
            ...
            if opt in Config:
                ...
                raise RuntimeError(
                    "Unknown option '%s' in [Options] section of configuration file" % opt)
    else:
...
#        yspacing = 3
    ...
    if do_abort:
        raise RuntimeError(
            'Exiting since jobs are missing layers. Set AllowMissingLayers=1\nto override.')
