...
PrimitiveParmTypes = (
    ...
    # Code 3  -- end-of-file for .DES files
    None,
    # Code 4  -- outline...takes any number of additional floats
    (int, int, float, float, float, float, float),
    # Code 5  -- regular polygon
    (int, int, float, float, float, float),
    (float, float, float, float, float, int,
     float, float, float),  # Code 6 -- moire
    ...
    # Code 20 -- line (vector)...alias for code 2
    (int, float, float, float, float, float),
    (int, float, float, float, float, float),         # Code 21 -- line (center)
    # Code 22 -- line (lower-left)
    (int, float, float, float, float, float)
)
...
class ApertureMacroPrimitive(object):
    ...
    def setFromFields(self, code, fields):
        ...
        if valids is None:
            raise RuntimeError(
                'Undefined aperture macro primitive code %d' % code)
        ...
        if self.code == 4:
            if len(fields) < 2:
                raise RuntimeError(
                    'Outline macro primitive has way too few fields')
    ...
    def rotate(self):
        ...
        # Polygon: fields (2,3) must be rotated, and field 5 incremented by +90
            ...
            rotatexypair(self.parms, 2)
            rotatethelem(self.parms, 5)
        # Moire: fields (0,1) must be rotated, and field 8 incremented by +90
        elif self.code == 6:
            rotatexypair(self.parms, 0)
            rotatethelem(self.parms, 8)
        # Thermal: fields (0,1) must be rotated, and field 5 incremented by +90
        elif self.code == 7:
            rotatexypair(self.parms, 0)
            rotatethelem(self.parms, 5)
...
def parseApertureMacro(s, fid):
    ...
    if match:
        ...
        for line in fid:
            ...
            raise RuntimeError(
                "Premature end-of-file while parsing aperture macro")
    else:
        return None
...
if __name__ == "__main__":
    ...
    M.add(ApertureMacroPrimitive(
        2, ('1', '0.0025', '0.0', '-0.1', '0.0', '0.1', '90.0')))
    ...
    M.add(ApertureMacroPrimitive(6, ('0.07', '0.07', '0.04',
                                     '0.005', '0.01', '2', '0.005', '0.04', '0.0')))
