...
def left_of(p1, p2):
    '''Helper function to determine if points are left of each other.'''
    '''This assumes the points are on a line which is vertial or horizontal'''
    return p1[0] < p2[0] and p1[1] == p2[1]


def right_of(p1, p2):
    '''Helper function to determine if points are right of each other.'''
    '''This assumes the points are on a line which is vertial or horizontal'''
    return p1[0] > p2[0] and p1[1] == p2[1]


def above(p1, p2):
    '''Helper function to determine if points are above each other.'''
    '''This assumes the points are on a line which is vertial or horizontal'''
    return p1[1] > p2[1] and p1[0] == p2[0]


def below(p1, p2):
    '''Helper function to determine if points are below each other.'''
    '''This assumes the points are on a line which is vertial or horizontal'''
    return p1[1] < p2[1] and p1[0] == p2[0]
...
class Tiling(object):
    ...
    def isOverlap(self, ix, X, Y, cfg=config.Config):
        ...
        for t_bl, t_tr, Job in self.jobs:
            if p_bl[0] < t_tr[0] and p_tr[0] > t_bl[0] and \
